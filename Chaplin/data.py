...
class TrajectoryIndexDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        values= data["value"][start_pos:end_pos]
        log_probs = data["log_prob"][start_pos:end_pos]
        rewards = data["reward"][start_pos:end_pos]
        return (
            torch.tensor(obs, dtype=torch.float32, device=self.device),
            torch.tensor(actions, dtype=torch.float32, device=self.device).view(
                self.batch_length, -1),
            torch.tensor(values, dtype=torch.float32, device=self.device).view(
                self.batch_length, -1),
...
class ExperienceReplayDataset:
    ...
    def add_trajectory(self, data: Dict):
        obs, actions, values,log_prob, rewards = data["obs"], data["action"], data["value"], data["log_prob"], data["reward"]
        episode_length = len(obs)
        assert episode_length == len(actions) == len(rewards) == len(values) == len(log_prob)
        ...
        self.values[self.oldest_index, :episode_length] = np.asarray(values)
        self.log_prob[self.oldest_index, :episode_length] = np.asarray(log_prob)
    ...
    def __next__(self, update_type='dreamer'):
    ...
    def collect(
        ...
        while num_episodes < target_num_episodes or num_steps < target_num_steps:
            if random_action:
                ...
                # log_prob = F.softmax(action, dim=-1)  # Convert logits to probabilities
                ...
                action_clipped = torch.clamp(action_tensor, min=-0.99, max=0.99)  # Adjust the range based on your specific action space
    ...
    def collect(
        ...
        while num_episodes < target_num_episodes or num_steps < target_num_steps:
            if random_action:
                ...
                log_prob = np.zeros((envs.num_envs, envs.action_space.shape[0]))  # Properly shaped as two-dimensional
