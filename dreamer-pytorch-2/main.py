...
parser.add_argument(
    "--model_learning-rate", type=float, default=1e-3, metavar="α", help="Learning rate"
)
parser.add_argument(
    "--actor_learning-rate", type=float, default=8e-5, metavar="α", help="Learning rate"
)
parser.add_argument(
    "--value_learning-rate", type=float, default=8e-5, metavar="α", help="Learning rate"
)
parser.add_argument(
...
parser.add_argument(
    "--models", type=str, default="", metavar="M", help="Load model checkpoint"
)
parser.add_argument(
    "--experience-replay",
    type=str,
...
free_nats = torch.full(
    (1,), args.free_nats, device=args.device
...
for episode in tqdm(
    ...
    losses = []
    model_modules = (
        transition_model.modules
        + encoder.modules
        + observation_model.modules
        + reward_model.modules
    )
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        if args.learning_rate_schedule != 0:
            for group in model_optimizer.param_groups:
                group["lr"] = min(
                    group["lr"]
                    + args.model_learning_rate / args.model_learning_rate_schedule,
    ...
    lineplot(
        metrics["episodes"][-len(metrics["actor_loss"]) :],
        metrics["actor_loss"],
        "actor_loss",
        results_dir,
    )
    ...
    writer.add_scalar(
        "observation_loss", metrics["observation_loss"][0][-1], metrics["steps"][-1]
    )
    writer.add_scalar(
