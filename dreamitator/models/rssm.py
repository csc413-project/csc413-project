...
class RSSM(nn.Module):
    ...
    def __init__(
        ...
        # imitator
        self.imitator_cell = nn.GRUCell(
            self.deter_size + reward_size, deterministic_size
        )
    ...
    def forward(
        ...
        return self.observe(steps, obs_embed, action, rewards, prev_state)
    ...
    def observe(
        ...
        :param rewards: size(time_steps, batch_size, reward_size)
        ...
        posteriors = []
        imitator_states = []
        prev_imitator_state = None
        for t in range(steps):
            prior_state = self.get_prior(action[t], prev_state)
            posterior_state = self.get_posterior(obs_embed[t], prior_state)
            imitator_state = self.get_imitator_state(
                rewards[t], prev_state, prev_imitator_state
            )

            priors.append(prior_state)
            posteriors.append(posterior_state)
            imitator_states.append(imitator_state)
            prev_state = posterior_state
            prev_imitator_state = imitator_state
        prior = stack_states(priors, dim=0)
        post = stack_states(posteriors, dim=0)
        return prior, post, torch.stack(imitator_states, dim=0)
    ...
    def get_imitator_state(
        self, reward: torch.Tensor, prev_state: RSSMState, prev_imitator: torch.Tensor
    ):
        imitator_input = torch.cat([prev_state.deter, reward], dim=-1)
        imitator_state = self.imitator_cell(imitator_input, prev_imitator)
        return imitator_state
