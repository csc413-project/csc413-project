...
class Dreamer:
    ...
    def calculate_loss(self, observations, actions, rewards):
        ...
        prior, posterior, imitator_state = self.agent.rssm.observe(
            seq_len, obs_embed, actions, rewards, prev_state
        ...
        with torch.no_grad():
            wandb.log(
                {
                    ...
                    "train/imitator_loss": imitator_loss.item(),
            ...
            if self.training_steps % 500 == 0:
                ...
                _, posterior, _ = self.agent.rssm.observe(
                    5,
                    obs_embed.reshape(5, 1, -1),
                    actions[:5, i].reshape(5, 1, -1),
                    rewards[:5, i].reshape(5, 1, -1),
                    prev_state,
                )
