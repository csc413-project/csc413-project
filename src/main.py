...
class TrajectoryDataset(Dataset):
    def __init__(self, data_dir, batch_length):
    ...
    def _update_file_list(self):
        # Dynamically load all file names
        new_files = [
            os.path.join(self.data_dir, f)
            for f in os.listdir(self.data_dir)
            if f.endswith(".npz")
        ]
        if set(new_files) != set(self.files):
    ...
    def __len__(self):
        self._update_file_list()  # Ensure the file list is up-to-date
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.transpose(torch.tensor(obs, dtype=torch.float32), 1, 3),
            torch.tensor(actions, dtype=torch.float32).view(self.batch_length, -1),
...
def main():
    ...
    while num_episodes < 1 and env_steps < 2000:
        ...
        pbar.update(1)
    pbar.close()
    np.savez_compressed(os.path.join(config.data_dir, "debug_data.npz"), **data)
