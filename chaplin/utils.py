...
def discounted_cumulative_sum(x: np.ndarray, discount: float, axis: int = -1):
    """
    Computes the discounted cumulative sums of vectors.

    The discount factor applies progressively, reducing the contribution of values further along in the sequence.
    This operation is equivalent to calculating the present value of future rewards.

    Parameters:
        x (np.ndarray): A multi-dimensional array where the discount cumulative sum is applied along the specified axis.
        discount (float): The discount factor applied to future elements in the sequence. A value of 0 results in no future
                          values being considered, while a value close to 1 gives them almost equal weight as immediate values.
        axis (int, optional): The axis along which the discounted cumulative sum is computed. Defaults to -1 (the last axis).

    Returns:
        np.ndarray: An array of the same shape as `x`, containing the discounted cumulative sums.

    Examples:
        >>> import numpy as np
        >>> discounted_cumulative_sum(np.array([1, 2, 3]), 0.5)
        array([1.75, 2.5 , 3.  ])
        >>> discounted_cumulative_sum(np.array([[1, 2, 3], [4, 5, 6]]), 0.9)
        array([[1.71, 4.9 , 3.  ], [8.26, 10.5, 6.  ]])
        >>> discounted_cumulative_sum(np.array([[1, 2], [3, 4], [5, 6]]), 0.5, axis=0)
        array([[3.25, 5.  ], [5.5 , 7.  ], [5.  , 6.  ]])

    Note: The output values in the doctests are rounded for readability. Actual outputs might have more decimal places.
    """
    ...
    discounted_sum_reversed = scipy.signal.lfilter(
        [1], [1, -discount], x_reversed, axis=axis
    )
    discounted_sum = np.flip(discounted_sum_reversed, axis=axis)
